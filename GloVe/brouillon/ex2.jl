...
function matrice2(n::Int)
    m = [[0.0 for i in 1:n] for j in 1:n]
    for i in 1:n
        for j in i:n
            r = rand(1:6)
            if r > 4
...
#= typeof(M1)==typeof(MA1)
M1 = matrice2(V)
M2 = matrice2(V)
@time Flux.train!(cost2, p, [M1], opt) 

g = Glove(64,10)
p = params(g)
opt = Flux.Optimise.ADAGrad()
Flux.train!(cost2, p, Iterators.repeated(M1, 50), opt)
...
g = Glove(64,10)
p = params(g)
Flux.train!(cost2, p, Iterators.repeated(M2, 50), opt)

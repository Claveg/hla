...
function Wv(vocsize::Int,hidsize::Int)
    Wv( (rand(Float64,hidsize,vocsize).-0.5) / (vocsize+1), rand(Float64,vocsize,hidsize,).-0.5 / (vocsize+1))
end
...
function ht(v::Int,n::Int) # matrice de n vecteurs h 
    l = []

    for i in 1:n
        vec = zeros(v)
        rd = sample(1:v, C, replace = false) # C indices différents entre 1:v
        for r in rd
            vec[r] = 1
        end
        push!(l,vec)
...
function dt(corpusSize::Int) #tupple de deux matrices: X vecteurs h (contexte) et Y vecteur main associé
